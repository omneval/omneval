...
- [ ] Lake-native retention enforcement (#92 — fixes latent bug: legacy retention worker scans `parquet/` while the archive lives under `archive/`, so it has never deleted anything; PR #115 adds `quack.server.retention.*` config and Table Maintenance retention step)

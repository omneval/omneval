...
### Trace
A group of Spans that share a `TraceID`. Has a single root Span (the one with no `ParentID`). Visualized as a waterfall in the UI. Multiple Traces may belong to the same Conversation.

...
`Input` is a JSON array of message objects following OTel GenAI semantic conventions: `[{"role":"system","content":"..."},{"role":"user","content":"..."}]`. The system prompt is included as the first element by default; this can be suppressed via `LANTERN_INGEST_LOG_SYSTEM_PROMPT=false`. `Output` is a JSON array of completion message objects: `[{"role":"assistant","content":"..."}]`. This format is intentionally aligned with OTel GenAI conventions — Lantern extends them where necessary but does not deviate from them.
...
- **Ingest API**: `lantern_ingest_spans_received_total{project_id}`, `lantern_ingest_enqueue_errors_total`, `lantern_ingest_request_duration_seconds`
- **Writer Service**: `lantern_writer_spans_written_total{project_id}`, `lantern_writer_duckdb_write_duration_seconds`, `lantern_writer_snapshot_sync_duration_seconds`, `lantern_writer_archive_spans_total{project_id}`
- **Query API**: `lantern_query_request_duration_seconds{endpoint}`, `lantern_query_snapshot_age_seconds`
- **Eval Workers**: `lantern_eval_jobs_processed_total{project_id,rule_id}`, `lantern_eval_job_duration_seconds`, `lantern_eval_judge_errors_total`

### Ingest Queue
Redis List at key `lantern:ingest:spans`. Ingest API translates OTLP→`domain.Span` and `RPUSH`es one JSON-encoded `[]*domain.Span` batch per request. Writer Service `BLPOP`s batches and writes to DuckDB. Translation happens in the Ingest API so the queue payload is always in the internal domain format regardless of whether spans arrived via OTLP or the native REST API.

### Eval Queue
Redis List at key `lantern:eval:jobs`. Writer Service `RPUSH`es one `domain.EvalJob` per entry after writing spans. Eval Workers `BLPOP` jobs and execute LLM-as-a-Judge evaluations.

Both queues share a single Redis instance configured via `redis` in `lantern.yaml`. No separate Redis topology for ingest vs. eval in Phase 1.

### Pricing Table
Loaded at Writer Service startup by fetching LiteLLM's `model_prices_and_context_window.json`. Falls back to a bundled snapshot embedded in the binary (`internal/pricing/model_prices_and_context_window.json`) if the live fetch fails. Prices are stored internally as USD per token. Config overrides (`lantern.yaml`) are expressed in USD per million tokens (human-readable) and converted to per-token at load time. Cost formula: `cost_usd = (input_tokens × input_per_token) + (output_tokens × output_per_token)`. `CostUSD` is computed in the Writer Service when writing a span to DuckDB — pre-computed at write time so query-time aggregations need no re-computation. Unknown models store `CostUSD = 0`.

### OTLP Translation
The `internal/otlp` package translates `ResourceSpans` into `domain.Span` values. The Ingest API accepts OTLP at `POST /v1/traces` as both `Content-Type: application/x-protobuf` (default for OTel SDKs) and `Content-Type: application/json`. Both encodings are decoded into the same `[]ResourceSpans` input before translation. Key mapping rules: `gen_ai.request.model`→`Model`; `gen_ai.usage.input_tokens` (or legacy `prompt_tokens`)→`InputTokens`; `gen_ai.usage.output_tokens` (or legacy `completion_tokens`)→`OutputTokens`; `gen_ai.prompt.N.{role,content}` arrays→`Input` JSON; `gen_ai.completion.N.{role,content}`→`Output` JSON; `lantern.prompt.name`/`lantern.prompt.version`→prompt linkage. `service.name` comes from the OTLP Resource; a service-scoped API key's `ServiceName` overrides it. `Kind` is derived: explicit `lantern.kind` attribute wins, then `gen_ai.*` presence→`llm`, then `tool.*` presence→`tool`, else `internal`. All unmapped attributes land in `Span.Attributes` overflow map.
...
### API Key
Two kinds: `ltn_proj_<43 base58 chars>` (project-scoped write access) and `ltn_svc_<43 base58 chars>` (service-scoped, attaches a `service_name` label to every span). Generated with 32 bytes of `crypto/rand` encoded as base58. Only the SHA-256 hex hash is stored in the metadata store — the raw key is shown to the user exactly once at creation. The Ingest API validates via a `CachingValidator` with a 60-second TTL; a revoked key may be accepted for up to 60 seconds after revocation.

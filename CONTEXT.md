...
### OTLP Translation
The `internal/otlp` package translates `ResourceSpans` into `domain.Span` values. The Ingest API accepts OTLP at `POST /v1/traces` as both `Content-Type: application/x-protobuf` (default for OTel SDKs) and `Content-Type: application/json`. Both encodings are decoded into the same `[]ResourceSpans` input before translation. Key mapping rules: `gen_ai.request.model`→`Model`; `gen_ai.usage.input_tokens` (or legacy `prompt_tokens`)→`InputTokens`; `gen_ai.usage.output_tokens` (or legacy `completion_tokens`)→`OutputTokens`; `gen_ai.prompt.N.{role,content}` arrays→`Input` JSON; `gen_ai.completion.N.{role,content}`→`Output` JSON; `omneval.prompt.name`/`omneval.prompt.version`→prompt linkage. `service.name` comes from the OTLP Resource; a service-scoped API key's `ServiceName` overrides it. `Kind` is derived via `deriveKind`, checked in order: explicit `omneval.kind` attribute wins; then OpenInference `openinference.span.kind` (`AGENT`→`agent`; `LLM`→`llm`; `TOOL`→`tool`; `CHAIN`/`RETRIEVER`/`EMBEDDING`/`RERANKER`/`GUARDRAIL`→`chain`); then OTel GenAI semconv `gen_ai.operation.name` (`invoke_agent`/`create_agent`→`agent`; `execute_tool`→`tool`; `chat`/`text_completion`/`embeddings`/`generate_content`→`llm`); then legacy `gen_ai.*`/`llm.*` presence→`llm`, `tool.*` presence→`tool`; then span-name pattern fallback (name ending in `.step` or containing "agent"→`agent`; name ending in `Action` or containing "tool"→`tool`); else `internal`. All unmapped attributes land in `Span.Attributes` overflow map.
...
### Disaster Recovery
The Lake's span/score data lives entirely on S3, so its durability is S3 durability. The Catalog (a local DuckDB file by default, see ADR-0006) lives on the Quack Server's PVC; backing it up is the operator's responsibility via standard PVC backup tooling (e.g. a VolumeSnapshot policy), not an Omneval-owned mechanism (ADR-0007). A Quack Server pod restart (eviction, OOM, crash) with the PVC intact needs no recovery action — Kubernetes reattaches the same PVC and the Quack Server simply reopens the existing Catalog file, with DuckDB replaying its own WAL for crash consistency. Recovery from a lost or corrupted PVC means restoring the operator's own backup; absent one, the documented "Rebuild Catalog from Parquet" runbook is the last resort (discarding snapshot/time-travel history). Unprocessed batches survive in the Ingest Buffer and are replayable.

...
describe("focus ring colors", () => {
  it("exports focus ring colors on the colors object", () => {
    ...
    expect(colors.focusRing.normal).toBe("rgba(124, 58, 237, 0.3)");
    expect(colors.focusRing.danger).toBe("rgba(239, 68, 68, 0.3)");
  });

  it("focus ring normal matches violet primary at 30% opacity", () => {
    const r = parseInt("7C", 16);
    const g = parseInt("3A", 16);
    const b = parseInt("ED", 16);
    expect(colors.focusRing.normal).toBe(`rgba(${r}, ${g}, ${b}, 0.3)`);
  });

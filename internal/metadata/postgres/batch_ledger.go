...
func (s *Store) MarkBatchCommitted(ctx context.Context, batchID string, committedAt time.Time) error {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO committed_batches (batch_id, committed_at)
		 VALUES ($1, $2)
		 ON CONFLICT (batch_id) DO NOTHING`,
		batchID, committedAt,
	)
	if err != nil {
		return fmt.Errorf("postgres: mark batch committed: %w", err)
	}
	return nil
}
...
func (s *Store) IsBatchCommitted(ctx context.Context, batchID string) (bool, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT count(*) FROM committed_batches WHERE batch_id = $1`,
		batchID,
	).Scan(&count)
	if err != nil {
		return false, fmt.Errorf("postgres: is batch committed: %w", err)
	}
	return count > 0, nil
}
